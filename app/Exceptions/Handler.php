...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $e)
    {
        ...
        if ($e instanceof BaseException) {
            $detail = '';
            foreach ($e->getReason() as $key => $value) {
                !empty($value) && $detail .= $key . ':' . $value . ', ';
            }

            !empty($detail) && $detail = substr($detail, 0, -2);
        ...
        if (!empty($exceptionData)) {
            if (isset($exceptionData['detail']) && !empty($exceptionData['detail'])) {
                return $this->exceptionResponse($request, $exceptionData['error_code'], $exceptionData['http_code'], $exceptionData['message'], $exceptionData['detail']);
            }
